...
if [ ! -e $_LOCAL_BIN/startsap.sh ]; then
    ...
    cp $NW752_SCRIPT_HOME/bin/startsap.sh $_LOCAL_BIN
    cp $NW752_SCRIPT_HOME/bin/stopsap.sh $_LOCAL_BIN
...
if [ -e /usr/sap/NPL/D00/exe/sapstart ] && [ ! -e $_SERVICE_PATH ]; then
    ...
    cp $NW752_SCRIPT_HOME/service/sapnw.service /etc/systemd/system/
